...
// PauseCommand is the command for pausing a service
...
func (c *PauseCommand) Examples() map[string]string {
	appName := os.Getenv("CLI_APP_NAME")
	return map[string]string{
		"Pauses a redis service named test": fmt.Sprintf("%s %s redis test", appName, c.Name()),
	}
}

// Arguments returns the arguments for the command
func (c *PauseCommand) Arguments() []command.Argument {
	args := []command.Argument{}
	args = append(args, command.Argument{
		Name:        "datastore-type",
		Description: "the type of datastore to pause",
		Optional:    false,
...
func (c *PauseCommand) ParsedArguments(args []string) (map[string]command.Argument, error) {
	return internal.ParseArguments(args, c.Arguments())
}

// FlagSet returns the flag set for the command
func (c *PauseCommand) FlagSet() *flag.FlagSet {
...
func (c *PauseCommand) Run(args []string) int {
	...
	err = service.PauseServiceContainer(ctx, service.PauseServiceContainerInput{
		Datastore:   datastore,
	...
	logger.Header1(fmt.Sprintf("Service %s paused", serviceName))
