...
// LinksCommand is the command for listing all apps that are linked to a given service
...
func (c *LinksCommand) Synopsis() string {
	return "Lists all apps that are linked to a given service"
}

// Help returns the help text for the command
func (c *LinksCommand) Help() string {
	return command.CommandHelp(c)
}

// Examples returns the examples for the command
func (c *LinksCommand) Examples() map[string]string {
	appName := os.Getenv("CLI_APP_NAME")
	return map[string]string{
		"Lists all apps that are linked to a redis service named test": fmt.Sprintf("%s %s redis test", appName, c.Name()),
	}
}

// Arguments returns the arguments for the command
func (c *LinksCommand) Arguments() []command.Argument {
	args := []command.Argument{}
	args = append(args, command.Argument{
		Name:        "datastore-type",
		Description: "the type of datastore to list the apps that are linked to a service",
		Optional:    false,
		Type:        command.ArgumentString,
	})
	args = append(args, command.Argument{
		Name:        "service-name",
		Description: "the name of the service to list the apps that are linked to",
		Optional:    false,
		Type:        command.ArgumentString,
	})
	return args
...
func (c *LinksCommand) AutocompleteArgs() complete.Predictor {
	return complete.PredictSet("redis")
}

// ParsedArguments parses the arguments for the command
func (c *LinksCommand) ParsedArguments(args []string) (map[string]command.Argument, error) {
	return command.ParseArguments(args, c.Arguments())
}

// FlagSet returns the flag set for the command
func (c *LinksCommand) FlagSet() *flag.FlagSet {
...
func (c *LinksCommand) AutocompleteFlags() complete.Flags {
...
func (c *LinksCommand) Run(args []string) int {
	...
	if err := logger.Table(fmt.Sprintf("%v linked apps for service %s", datastoreType, serviceName), linkedApps); err != nil {
		logger.Error(internal.ErrorInput{
			Error: err,
		})
		return 1
